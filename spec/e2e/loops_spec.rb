...
RSpec.describe "Loops", type: [:standard, :e2e] do
  ...
  #   services[:NestedService] = Class.new do
  ...
  #             Run step `:iterate` (steps[0]).
  #             Run step `NestedService` (steps[0][0]).
  #             Run step `NestedService` (steps[0][1]).
  ...
  example_group "instance methods" do
    ...
    describe "#all?" do
      specify do
        ...
        # no block
        expect([:success, :success, :success].all?).to eq(true)
        expect(service.collection([:success, :success, :success]).all?.result).to be_success.without_data

        # pattern
        expect([:failure, :success, :exception].all?(/success/)).to eq(false)
        expect(service.collection([:failure, :success, :exception]).all?(/success/).result).to be_failure.without_data

        # not satisfied condition
        expect([:success, :failure, :exception].all?(&conditional_block)).to eq(false)
        expect(service.collection([:success, :failure, :exception]).all?(&conditional_block).result).to be_failure.without_data
        expect(service.collection([:success, :failure, :exception]).all?(&step_without_outputs_block).result).to be_failure.without_data

        # satisfied condition
        expect([:success, :success, :success].all?(&conditional_block)).to eq(true)
        expect(service.collection([:success, :success, :success]).all?(&conditional_block).result).to be_success.without_data
        expect(service.collection([:success, :success, :success]).all?(&step_without_outputs_block).result).to be_success.without_data

        # error result
        expect(service.collection([:success, :error, :exception]).all?(&step_without_outputs_block).result).to be_error.without_data
