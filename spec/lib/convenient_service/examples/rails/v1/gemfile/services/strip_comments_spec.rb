...
RSpec.describe ConvenientService::Examples::Rails::V1::Gemfile::Services::StripComments, type: :standard do
    ...
    describe ".result" do
      ...
      context "when `StripComments` is NOT successful" do
        ...
        context "when `strip-comments` npm package is not available" do
          before do
            stub_service(ConvenientService::Examples::Rails::V1::Gemfile::Services::AssertNpmPackageAvailable)
              .with_arguments(name: npm_package_name)
              .to return_error
          end

          it "returns intermediate step result" do
            expect(result).to be_not_success.of_step(ConvenientService::Examples::Rails::V1::Gemfile::Services::AssertNpmPackageAvailable)
          end
        end
      end
      ...
      context "when `StripComments` is successful" do
        before do
          ...
          #
          stub_service(ConvenientService::Examples::Rails::V1::Gemfile::Services::RunShellCommand).to return_success
        end
