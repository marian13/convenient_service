...
RSpec.describe ConvenientService::Examples::Dry::Gemfile::Services::PrintShellCommand, type: :standard do
      ...
      context "when `PrintShellCommand` is NOT successful" do
        context "when command is NOT present" do
          ...
          it "returns `error` with `message`" do
            expect(result).to be_error.with_message("command must be filled")
          end
        end
