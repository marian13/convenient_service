...
RSpec.describe ConvenientService::Examples::Gemfile::Services::ParseContent do
  ...
  describe "#result" do
    ...
    context "when content has invalid Ruby syntax" do
      before do
        stub_service(ConvenientService::Examples::Gemfile::Services::AssertValidRubySyntax)
          .with_arguments(content: content)
          .to return_error
      end

      it "returns intermediate error" do
        expect(result).to be_error.of(ConvenientService::Examples::Gemfile::Services::AssertValidRubySyntax)
      end
    end
