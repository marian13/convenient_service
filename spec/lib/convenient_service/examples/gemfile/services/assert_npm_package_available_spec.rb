...
RSpec.describe ConvenientService::Examples::Gemfile::Services::AssertNpmPackageAvailable do
  ...
  describe "#result" do
    ...
    before do
      stub_service(ConvenientService::Examples::Gemfile::Services::AssertNodeAvailable).to return_error
    end

    context "when node is NOT available" do
      it "returns intermediate error" do
        expect(result).to be_error.of(ConvenientService::Examples::Gemfile::Services::AssertNodeAvailable)
      end
    end
    ...
    context "when node is available" do
      ...
      before do
        stub_service(ConvenientService::Examples::Gemfile::Services::AssertNodeAvailable).to return_success
      end

      context "when npm package is NOT available" do
        before do
          stub_service(ConvenientService::Examples::Gemfile::Services::RunShell)
            .with_arguments(command: npm_package_available_command)
            .to return_error
        end

        it "returns intermediate error" do
          expect(result).to be_error.of(ConvenientService::Examples::Gemfile::Services::RunShell)
        end
      end
      ...
      context "when npm package is available" do
        ...
        it "returns success" do
          expect(result).to be_success.of(ConvenientService::Examples::Gemfile::Services::RunShell)
        end
      end
    end
  end
