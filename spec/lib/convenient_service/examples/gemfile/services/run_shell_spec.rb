...
RSpec.describe ConvenientService::Examples::Gemfile::Services::RunShell do
  ...
  describe "#result" do
    ...
    context "when shell command has non-zero code" do
      ...
      context "when debug is truthy" do
        ...
        before do
          allow(ConvenientService::Examples::Gemfile::Services::PrintShellCommand).to receive(:result).with(hash_including(text: command))
        end

        it "prints command" do
          result

          expect(ConvenientService::Examples::Gemfile::Services::PrintShellCommand).to have_received(:result)
        end
      end
