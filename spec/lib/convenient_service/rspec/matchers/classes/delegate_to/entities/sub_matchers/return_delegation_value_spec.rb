...
RSpec.describe ConvenientService::RSpec::Matchers::Classes::DelegateTo::Entities::SubMatchers::ReturnDelegationValue, type: :standard do
  ...
  example_group "inheritance" do
    ...
    it { is_expected.to be_descendant_of(ConvenientService::RSpec::Matchers::Classes::DelegateTo::Entities::SubMatchers::Base) }
  end
  ...
  example_group "instance methods" do
    ...
    describe "#failure_message" do
      ...
      let(:message) do
        <<~MESSAGE.chomp
          expected `#{matcher.inputs.printable_block_expectation}` to delegate to `#{matcher.inputs.printable_method}` and return its value, but it didn't.

          `#{matcher.inputs.printable_block_expectation}` returns `#{block_expectation_value.inspect}`, but delegation returns `#{delegation_value.inspect}`.
        MESSAGE
      end
      ...
      context "when block expectation value and return value of method invocation on object have same visual output" do
        ...
        let(:message_with_note) do
          <<~MESSAGE.chomp
            ...
            NOTE: Block expectation value `#{block_expectation_value.inspect}` and delegation value `#{delegation_value.inspect}` have the same visual output, but they are different objects in terms of `#==`.
    ...
    describe "#failure_message_when_negated" do
      ...
      let(:message) { "expected `#{matcher.inputs.printable_block_expectation}` NOT to delegate to `#{matcher.inputs.printable_method}` and return its value, but it did." }
