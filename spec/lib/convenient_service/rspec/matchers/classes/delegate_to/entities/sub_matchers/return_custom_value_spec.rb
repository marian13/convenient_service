...
RSpec.describe ConvenientService::RSpec::Matchers::Classes::DelegateTo::Entities::SubMatchers::ReturnCustomValue, type: :standard do
  let(:sub_matcher) { described_class.new(matcher: matcher) }

  let(:matcher) { ConvenientService::RSpec::Matchers::Classes::DelegateTo.new(object, method).and_return { |delegation_value| "#{delegation_value} + custom return value" } }
  ...
  example_group "instance methods" do
    ...
    describe "#failure_message" do
      ...
      context "when block expectation value and return custom value have same visual output" do
        let(:matcher) { ConvenientService::RSpec::Matchers::Classes::DelegateTo.new(object, method).and_return { "same visual output" } }
