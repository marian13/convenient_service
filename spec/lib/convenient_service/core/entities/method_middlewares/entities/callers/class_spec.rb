...
RSpec.describe ConvenientService::Core::Entities::MethodMiddlewares::Entities::Callers::Class do
    ...
    describe "#commit_config!" do
      specify do
        expect { caller.commit_config! }
          .to delegate_to(entity, :commit_config!)
    ...
    describe "#ancestors" do
      specify do
        expect { caller.ancestors }
          .to delegate_to(entity.singleton_class, :ancestors)
          .and_return_its_value
      end
    end
    ...
    describe "#methods_middlewares_callers" do
      specify do
        expect { caller.methods_middlewares_callers }
          ...
          .with_arguments(entity, :ClassMethodsMiddlewaresCallers)
          .and_return_its_value
      end
    end
    ...
    describe "#resolve_super_method" do
      ...
      context "when one ancestor from `ancestors_greater_than_methods_middlewares_callers` has own method" do
        ...
        it "returns own method" do
          expect(caller.resolve_super_method(method_name)).to eq(mod::ClassMethods.instance_method(method_name).bind(service_class))
        end
      end
      ...
      context "when multiple ancestors from `ancestors_greater_than_methods_middlewares_callers` have own method" do
        ...
        it "returns own method from lowest of them (closest from inheritance chain)" do
          expect(caller.resolve_super_method(method_name)).to eq(other_mod::ClassMethods.instance_method(method_name).bind(service_class))
        end
      end
    end
  end
