...
RSpec.describe ConvenientService::Support::Cache::Entities::Caches::ThreadSafeHash, type: :standard do
  ...
  example_group "instance methods" do
    ...
    describe "#backend" do
      ...
      it "returns `ConvenientService::Support::Cache::Constants::Backends::THREAD_SAFE_HASH`" do
        expect(cache.backend).to eq(ConvenientService::Support::Cache::Constants::Backends::THREAD_SAFE_HASH)
      end
    end
        ...
        context "when `backed_by` option is passed" do
          context "when cache does NOT sub cache by scope" do
            it "returns sub cache backed by `backed_by` option backend" do
              expect(cache.scope(:foo, backed_by: ConvenientService::Support::Cache::Constants::Backends::ARRAY).backend).to eq(ConvenientService::Support::Cache::Constants::Backends::ARRAY)
            end
          end

          context "when cache already has sub cache by scope" do
            before do
              cache.scope(:foo, backed_by: ConvenientService::Support::Cache::Constants::Backends::THREAD_SAFE_HASH).write(:bar, :baz)
            end

            it "returns sub cache with already set backend ignoring `backed_by` option" do
              expect(cache.scope(:foo, backed_by: ConvenientService::Support::Cache::Constants::Backends::ARRAY).backend).to eq(ConvenientService::Support::Cache::Constants::Backends::THREAD_SAFE_HASH)
            end
          end
        end
      end
