...
RSpec.describe ConvenientService::Service::Plugins::CanHaveServiceAwareEnumerables::Entities::ServiceAwareEnumerables::Base, type: :standard do
  include ConvenientService::RSpec::Helpers::IgnoringException

  include ConvenientService::RSpec::Matchers::DelegateTo
  include ConvenientService::RSpec::Matchers::Results

  let(:service_aware_base) { described_class.new(object: object, organizer: organizer, propagated_result: propagated_result) }
  ...
  example_group "instance methods" do
    ...
    describe "#result" do
      ...
      let(:service_aware_descendant) { service_aware_klass.new(object: object, organizer: organizer, propagated_result: propagated_result) }

      let(:data_key) { :elements }
      let(:evaluate_by) { :to_s }

      let(:result) { service_aware_descendant.result(data_key: data_key, evaluate_by: evaluate_by) }
      ...
      context "when step aware descendant does NOT have propagated result" do
        ...
        context "when `data_key` is NOT passed" do
          let(:result) { service_aware_descendant.result(evaluate_by: evaluate_by) }

          it "defaults to `default_data_key`" do
            expect(result).to be_success.with_data(service_aware_descendant.default_data_key => object.to_s).of_service(service).without_step
          end
        end
        ...
        context "when `evaluate_by` is NOT passed" do
          let(:result) { service_aware_descendant.result(data_key: data_key) }

          it "defaults to `default_evaluate_by`" do
            expect { result }
              .to delegate_to(object, service_aware_descendant.default_evaluate_by)
              .without_arguments
          end
        end
        ...
        example_group "`evaluate_by` option" do
          context "when `evaluate_by` is NOT valid" do
            ...
            it "raises `ConvenientService::Service::Plugins::CanHaveServiceAwareEnumerables::Exceptions::InvalidEvaluateByValue`" do
              expect { result }
                .to raise_error(ConvenientService::Service::Plugins::CanHaveServiceAwareEnumerables::Exceptions::InvalidEvaluateByValue)
                .with_message(exception_message)
            end

            specify do
              expect { ignoring_exception(ConvenientService::Service::Plugins::CanHaveServiceAwareEnumerables::Exceptions::InvalidEvaluateByValue) { result } }
    ...
    example_group "comparison" do
      describe "#==" do
        context "when `other` have different class" do
          ...
          it "returns `nil`" do
            expect(service_aware_base == other).to be_nil
          end
        end
        ...
        context "when `other` have different `object`" do
          ...
          it "returns `false`" do
            expect(service_aware_base == other).to be(false)
          end
        end
        ...
        context "when `other` has same attributes" do
          ...
          it "returns `true`" do
            expect(service_aware_base == other).to be(true)
          end
        end
      end
    end
