...
RSpec.describe ConvenientService::Service::Plugins::CanHaveServiceAwareEnumerables::Entities::ServiceAwareEnumerables::Set, type: :standard do
  include ConvenientService::RSpec::Matchers::DelegateTo

  let(:service_aware_set) { described_class.new(object: object, organizer: organizer, propagated_result: propagated_result) }
  ...
  example_group "instance methods" do
    ...
    describe "#default_data_key" do
      it "returns `:values`" do
        expect(service_aware_set.default_data_key).to eq(:values)
      end
    end

    describe "#default_evaluate_by" do
      it "returns `:to_set`" do
        expect(service_aware_set.default_evaluate_by).to eq(:to_set)
      end
    end
  end
end
