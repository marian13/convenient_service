...
RSpec.describe ConvenientService::Service::Plugins::CanHaveServiceAwareEnumerables::Entities::ServiceAwareEnumerables::Boolean, type: :standard do
  include ConvenientService::RSpec::Matchers::DelegateTo
  include ConvenientService::RSpec::Matchers::Results

  let(:service_aware_boolean) { described_class.new(object: object, organizer: organizer, propagated_result: propagated_result) }
  ...
  example_group "inheritance" do
    ...
    it { is_expected.to be_descendant_of(ConvenientService::Service::Plugins::CanHaveServiceAwareEnumerables::Entities::ServiceAwareEnumerables::Object) }
  end
  ...
  example_group "instance methods" do
    example_group "alias methods" do
      ...
      subject { service_aware_boolean }

      it { is_expected.to have_alias_method(:boolean, :object) }
    end

    describe "#result" do
      let(:result) { service_aware_boolean.result }
      ...
      context "when step aware boolean does NOT have propagated result" do
        ...
        context "when `object` is `false`" do
          ...
          context "when `data_key` is passed" do
            let(:result) { service_aware_boolean.result(data_key: data_key) }
          ...
          context "when `evaluate_by` is passed" do
            let(:result) { service_aware_boolean.result(evaluate_by: evaluate_by) }
    ...
    describe "#default_data_key" do
      it "returns `nil`" do
        expect(service_aware_boolean.default_data_key).to be_nil
      end
    end

    describe "#default_evaluate_by" do
      it "returns `nil`" do
        expect(service_aware_boolean.default_evaluate_by).to be_nil
      end
    end
  end
end
