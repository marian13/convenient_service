    ...
    describe "#call" do
      ...
      context "when callback `method` is `:step`" do
        ...
        it "adds after `:result` callback to service step class" do
          ...
          #
          expect(service_class.step_class.callbacks.for([:after, :organizer_result])).not_to be_empty
        end

        it "passes original block source location to after `:result` callback for service step class" do
          method_value

          expect(service_class.step_class.callbacks.for([:after, :organizer_result]).first.source_location).to eq(block.source_location)
        end
