    ...
    describe ".stubbed_results" do
      context "when RSpec current example is NOT set" do
        ...
        it "returns empty cache" do
          expect(service_class.stubbed_results).to eq(ConvenientService::Support::Cache.default_class.new)
        end
      end

      context "when RSpec current example is set" do
        it "returns cache scoped by self" do
          expect(service_class.stubbed_results).to eq(ConvenientService::Support::Cache.default_class.new.scope(service_class))
        end
